...
from telegram_inside_chainlit import start_telegram_bot
...
# ---------------------------------------------------------------------------
# Force embedder + reranker to stay on CPU during EVERY call.
#
# FlagEmbedding sometimes auto-routes intermediate tensors to GPU during
# .encode() / .compute_score() even when the model weights are on CPU.
# We block this by temporarily hiding CUDA from torch for the duration of
# each call, then restoring it. ASR (which loads next) is untouched because
# the patch is only active during the wrapped call.
# ---------------------------------------------------------------------------
import functools
...
def _force_cpu_during_call(fn):
    """Wrap a method so torch.cuda is invisible while it runs."""
    @functools.wraps(fn)
    def wrapped(*args, **kwargs):
        ...
        try:
            ...
            torch.cuda.is_available = _real_cuda_is_available
    return wrapped


# Make sure model weights are explicitly on CPU.
try:
    if hasattr(embed_model, "model"):
        embed_model.model = embed_model.model.to("cpu")
    if hasattr(reranker, "model"):
        reranker.model = reranker.model.to("cpu")
except Exception as _e:
    print(f"[cpu-fence] could not force .to('cpu'): {_e}")

# Wrap the only two methods that actually run inference.
if hasattr(embed_model, "encode"):
    embed_model.encode = _force_cpu_during_call(embed_model.encode)
if hasattr(reranker, "compute_score"):
    reranker.compute_score = _force_cpu_during_call(reranker.compute_score)

# Clean up any GPU buffers FlagEmbedding may have briefly allocated.
gc.collect()
torch.cuda.empty_cache()
print("[cpu-fence] embedder + reranker locked to CPU for all calls")
...
# Startup VRAM snapshot. Should show only ASR on GPU now.
if torch.cuda.is_available():
    _alloc = torch.cuda.memory_allocated() / 1e9
    _reserved = torch.cuda.memory_reserved() / 1e9
    print(f"[VRAM:startup] alloc={_alloc:.2f}GB reserved={_reserved:.2f}GB (only ASR should be here)")
...
async def transcribe_audio(audio_file) -> str:
    ...
    async with gpu_lock:
        ...
        torch.cuda.empty_cache()  # release Whisper's decoder scratch
...
# ---------------------------------------------------------------------------
# Mount the Telegram bot inside this Chainlit process so ASR / embedder /
# reranker / FAISS / LLM client are loaded only ONCE. Idempotent: only the
# first call actually starts polling; later chat opens are no-ops.
# ---------------------------------------------------------------------------
_TG_KICKED = False
_TG_KICK_LOCK = asyncio.Lock()


async def _kick_telegram_bot_once():
    global _TG_KICKED
    async with _TG_KICK_LOCK:
        if _TG_KICKED:
            return
        try:
            await start_telegram_bot(asr=asr, gpu_lock=gpu_lock)
            _TG_KICKED = True
            print("[telegram] Mounted inside Chainlit. Polling started.")
        except Exception as e:
            import traceback
            print(f"[telegram] FAILED to mount: {e}")
            traceback.print_exc()


@cl.on_chat_start
async def on_chat_start():
    # First chainlit chat opened => boot the Telegram bot. Once kicked it
    # keeps polling for the whole life of the chainlit process.
    await _kick_telegram_bot_once()
