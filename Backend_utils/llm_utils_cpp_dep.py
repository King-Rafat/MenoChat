...
def llama_cpp_extra_body(repeat_penalty: float = 1.1) -> dict:
...
async def run_planner(session_id: str, user_message: str) -> dict:
	...
	gen_text = await llm_chat(
		...
		top_p=PLANNER_TOP_P,
		extra_body=llama_cpp_extra_body(1.1),
		tag="planner",
	)
...
async def _stream_answer(
	...
	stream = await _llm_client.chat.completions.create(
		...
		extra_body=llama_cpp_extra_body(1.15),
	)
