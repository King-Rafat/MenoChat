"""
Chainlit-side bridge to the denoise pipeline.

Spawns denoise_runner.py inside denoise_venv. Loud-logs every step so you
can actually see what is happening. Does NOT silently swap in the raw file
on failure unless you opt in with `fallback_to_raw=True`.
"""
...
DENOISE_RUNNER = os.environ.get(
    "DENOISE_RUNNER",
    os.path.join(_HERE, "denoise_runner.py"),
)
...
def preprocess_audio(
    ...
    Loud-logs to the chainlit terminal so you can see what really happened.
    On failure, falls back to copying the raw audio to `output_path` ONLY if
    `fallback_to_raw=True` (default). Set False if you want a hard error.
    """
    ...
    # Hard fail if the runner is not present. No silent skipping.
    missing = []
    ...
    if missing:
        for m in missing:
            _log(f"MISSING: {m}")
        if fallback_to_raw:
            _log("falling back to RAW audio (set fallback_to_raw=False to hard-fail)")
    ...
    # Always print stderr tail so we see warnings even on success.
    if rc.stderr.strip():
    ...
    # Sanity check: did the cleaned file end up identical to the input?
    in_size = os.path.getsize(input_path)
    out_size = os.path.getsize(output_path)
    _log(f"sizes: input={in_size}B  cleaned={out_size}B")
    if in_size == out_size:
        _log("WARNING: cleaned file is exactly the same size as input. Likely the fallback fired silently inside the runner.")

    return output_path


def smoke_test(input_path: str) -> None:
    """Quick sanity check you can call from a python REPL or a -c oneliner."""
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("usage: python denoise.py <input_audio_file>")
        sys.exit(2)
