...
from audio_denoise import preprocess_audio
import numpy as np
import torch
...
async def transcribe_audio(audio_file) -> str:
    ...
    async with gpu_lock:
        print("[transcribe_audio] calling preprocess_audio", flush=True)
        cleaned_path = await loop.run_in_executor(
            _EXECUTOR, preprocess_audio, temp_path, cleaned_path
        )
        print(f"[transcribe_audio] preprocess_audio returned: {cleaned_path}", flush=True)
        asr_result = await loop.run_in_executor(_EXECUTOR, asr, cleaned_path)
...
async def on_audio_end():
    ...
    try:
        ...
        async with cl.Step(name="🎤 Transcribing audio...") as step:
            async with gpu_lock:
                print("[on_audio_end] calling preprocess_audio", flush=True)
                cleaned_path = await loop.run_in_executor(
                    _EXECUTOR, preprocess_audio, temp_path, cleaned_path
                )
                print(f"[on_audio_end] preprocess_audio returned: {cleaned_path}", flush=True)
                asr_result = await loop.run_in_executor(_EXECUTOR, asr, cleaned_path)
